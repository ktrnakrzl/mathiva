                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        TweenAnimationBuilder<double>(
                          ...
                          child: Container(
                            width: 88,
                            height: 88,
                            decoration: BoxDecoration(gradient: LinearGradient(colors: [palette.primary, palette.secondary]), shape: BoxShape.circle),
                            child: const Icon(Icons.emoji_events_rounded, color: Color(0xFFF7F9FC), size: 44),
                          ),
                        ),
                        const SizedBox(height: 18),
                        ShaderMask(
                          shaderCallback: (bounds) => LinearGradient(colors: [palette.primary, palette.secondary]).createShader(bounds),
                          child: Text('${result.score}/${result.total}', style: const TextStyle(color: Colors.white, fontSize: 40, fontWeight: FontWeight.w900)),
                        ),
                        const SizedBox(height: 8),
                        Text(result.passed ? 'Passed' : 'Needs more practice', style: const TextStyle(color: _ink, fontWeight: FontWeight.w900, fontSize: 16)),
                        const SizedBox(height: 8),
                        Text('Points earned: ${result.points_earned}', style: const TextStyle(color: _muted)),
                      ]),
                    ),
                  ),
                ),
