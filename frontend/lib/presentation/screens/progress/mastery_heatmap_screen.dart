            ...
            data: (items) => CustomScrollView(
              ...
              slivers: [
                ...
                SliverPadding(
                  ...
                  sliver: SliverGrid(
                    gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 1.15, crossAxisSpacing: 14, mainAxisSpacing: 14),
                    delegate: SliverChildBuilderDelegate(
                      (context, index) {
                        final item = items[index];
                        return FadeSlideIn(
                          delay: Duration(milliseconds: 60 * index),
                          child: _SoftCard(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                            Text(item.topic_name, textAlign: TextAlign.center, style: const TextStyle(color: _ink, fontWeight: FontWeight.w900)),
                            const SizedBox(height: 10),
                            ShaderMask(
                              shaderCallback: (bounds) => LinearGradient(colors: [palette.primary, palette.secondary]).createShader(bounds),
                              child: Text('${item.mastery_level}%', style: const TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w900)),
                            ),
                            const SizedBox(height: 8),
                            _ProgressBar(value: item.mastery_level / 100, color: palette.primary),
                            const SizedBox(height: 8),
                            Text('Last: ${item.last_practiced}', textAlign: TextAlign.center, style: const TextStyle(color: _muted, fontSize: 11)),
                          ])),
                        );
                      },
...
class _ProgressBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ClipRRect(
        ...
        child: TweenAnimationBuilder<double>(
          ...
          builder: (context, v, _) => LinearProgressIndicator(value: v, minHeight: 9, backgroundColor: _chip, valueColor: AlwaysStoppedAnimation<Color>(color)),
        ),
      );
}
