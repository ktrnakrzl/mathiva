...
class MockQuizRepository implements QuizRepository {
  ...
  Future<QuizStartResponse> startQuiz(QuizStartRequest request) async {
    ...
    return QuizStartResponse(
      ...
      questions: [
        QuizQuestion(
            question_id: 'q1',
            question_text: 'Evaluate \\(f(2)\\) if \\(f(x)=2x+1\\).',
            choices: const ['3', '4', '5', '6']),
        QuizQuestion(
            question_id: 'q2',
            question_text: 'Which relation is a function?',
            choices: const [
  ...
  Future<AnswerResponse> submitAnswer(AnswerRequest request) async {
    ...
    final correct = request.student_answer == '5' ||
        request.student_answer == 'One input, one output';
    return AnswerResponse(
      is_correct: correct,
      correct_answer:
          request.question_id == 'q1' ? '5' : 'One input, one output',
      solution_steps: const [
